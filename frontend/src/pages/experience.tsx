...
const Experience = () => {
    ...
    const QuantHubData = {
        company: "QuantHub",
        role: "Software Development + Market Research Industry Project",
        description:
            "At QuantHub, I conducted market research to identify opportunities for expanding AI education across college campuses. I developed a webscraper that collects course registration data from the University of Alabama's course registration system. The scraper is used to collect data for an industry-sponsored project on the impact of AI on course registration.",
        ...
        projects: [
            {
                ...
                description:
                    "I developed a webscraper that collects course registration data from the University of Alabama's course registration system. The scraper is used to collect data for an industry-sponsored project on the impact of AI on course registration.",
    ...
    const alaaiExpanded = ALAAIData.projects.some(
        (card) => card.id === expandedCard,
    );
    const quantHubExpanded = QuantHubData.projects.some(
        (card) => card.id === expandedCard,
    );
        ...
        <div className="flex flex-col w-full h-full min-h-screen">
            ...
            <div className="flex flex-col justify-center items-center text-center w-full h-full px-8 py-4 transition duration-300 ease-in-out">
                ...
                <h3 className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold">
                    A showcase of my professional journey - roles,
                    responsibilities, and the skills I've sharpened along the
                    way. Each experience reflects growth, collaboration, and the
                    ability to adapt to new challenges.
                </h3>
                <h3 className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold">
                    Because the source code for these projects is proprietary, I
                    cannot provide a direct link to the Github repositories.
                    However, you can view the production versions of the
                    projects by clicking the "Production" button on each project
                    card. If you have any questions or would like to learn more
                    about a project, please feel free to{" "}
                    <Link
                        to="/contact"
                        className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold underline hover:font-bold"
                    >
                        contact me
                    </Link>
                    .
            ...
            <div className="flex w-full h-full lg:flex-row flex-col items-center justify-center transition duration-300 ease-in-out">
                <div className="flex flex-col items-center justify-center h-full lg:w-1/2 w-full px-8 py-4 text-center">
                    <h2 className="text-light-text-primary dark:text-dark-text-primary text-xl font-bold">
                        {ALAAIData.company}
                    </h2>
                    <h3 className="text-light-text-primary dark:text-dark-text-primary text-lg font-bold">
                        {ALAAIData.role}
                    </h3>
                    <p className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold">
                        {ALAAIData.time}
                    </p>
                    <p className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold">
                        {ALAAIData.description}
                    </p>
                </div>

                <div className="flex h-full lg:w-1/2 w-full">
                    {!alaaiExpanded ? (
                        <div className="grid h-full w-full grid-cols-2 auto-rows-min gap-4 m-4 px-4 items-center">
                            {ALAAIData.projects.map((card) => (
                                <ExperienceCard
                                    key={card.id}
                                    {...card}
            ...
            <div className="flex w-full h-full lg:flex-row flex-col items-center justify-center transition duration-300 ease-in-out">
                <div className="flex flex-col items-center justify-center h-full lg:w-1/2 w-full px-8 py-4 text-center">
                    ...
                    <p className="text-light-text-secondary dark:text-dark-text-secondary text-lg font-semibold">
                        {QuantHubData.description}
                    </p>
                </div>

                <div className="flex h-full lg:w-1/2 w-full">
                    {!quantHubExpanded ? (
                        <div className="grid h-full w-full grid-cols-1 auto-rows-min gap-4 m-4 px-4 items-center">
